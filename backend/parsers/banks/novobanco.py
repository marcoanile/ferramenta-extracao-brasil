...
class NovoBancoParser(BankParser):
    ...
    def parse_pdf(self, path, filename: str) -> ParsedStatement:
        ...
        self._filter_foreign_account_rows(stmt)
        return stmt

    def _filter_foreign_account_rows(self, stmt: ParsedStatement):
        """Drop rows whose running balance is inconsistent with the main account.

        Novo Banco PDFs can include sub-account sections (e.g., Juros / Imposto
        on a savings sub-account) whose rows share the table layout but report
        balances from a different account. We detect them by walking the
        movement list and rejecting any row whose `prev_balance + amount`
        diverges from the row's reported balance by more than 1 cent.
        """
        if stmt.opening_balance is None or not stmt.movements:
            return
        kept = []
        ...
        for m in stmt.movements:
            if m.balance is None:
                ...
                continue
            expected = round(running + m.amount, 2)
            if abs(expected - m.balance) <= 0.01:
                kept.append(m)
                running = m.balance
            else:
                stmt.warnings.append(
                    f"Movimento ignorado (saldo incoerente — possivel sub-conta): "
                    f"{m.date} {m.amount:+.2f} bal={m.balance:.2f} (esperado {expected:.2f})"
                )
        stmt.movements = kept
